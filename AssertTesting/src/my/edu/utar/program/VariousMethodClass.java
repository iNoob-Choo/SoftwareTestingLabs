...
public class VariousMethodClass {
	...
	// Given an integer array, find the largest number contained in that array
	 public int findLargestNumberInArray(int[] numArray) { 
		 int bigNum = numArray[0]; 
		 for (int i = 0; i < numArray.length; i++) 
			 if (bigNum <numArray[i]) bigNum = numArray[i]; 
		 return bigNum;
		 
	 } 
	 // Given an array of Strings and an integer x, calculate the average of the length of the
	 //  String elements in the array whose length is greater than x.
